...
//	 */
//	public static Double[] getRoadDirection2(LocationIndexedLine line, Coordinate c, char oneway) {
...
//	}
	...
	public static List<Double> getRoadDirection(LocationIndexedLine line, Coordinate c, char oneway) {
		...
		LinearLocation here = line.project(c);
		LinearLocation end = line.getEndIndex();
		LinearLocation llafter = new LinearLocation(here.getComponentIndex(), here.getSegmentIndex() + 1, 0);
		Double temp = null;
//		如果传入点不是终点
		if (llafter.getSegmentIndex() <= end.getSegmentIndex()) {
//		不是T意味着该道路可以正向行驶
			if (oneway != 'T') {
				temp = calcDirection(c, line.extractPoint(llafter));
				if (temp != null) {
					ans.add(temp);
				}
			}
//		不是F意味着该道路可以逆向行驶
			if (oneway != 'F') {
				temp = calcDirection(line.extractPoint(llafter), c);
				if (temp != null) {
					ans.add(temp);
				}
			}
		}
		if (here.getSegmentFraction() == 0 || llafter.getSegmentIndex() > end.getSegmentIndex()) {
			LinearLocation llpre = new LinearLocation(here.getComponentIndex(), here.getSegmentIndex() - 1, 0);
//			不是T意味着该道路可以正向行驶
			if (oneway != 'T') {
				temp = calcDirection(line.extractPoint(llpre), c);
				if (temp != null) {
					ans.add(temp);
				}
			}
//			不是F意味着该道路可以逆向行驶
			if (oneway != 'F') {
				temp = calcDirection(c, line.extractPoint(llpre));
				if (temp != null) {
					ans.add(temp);
				}
			}
